#!/usr/bin/env python3
"""
migrate-actor.py — Port a single actor from the old C3PO intel-repository
to the new Archimedes repo structure.

Usage:
    python scripts/migrate-actor.py <source-dir> <actor-id>

Example:
    python scripts/migrate-actor.py \\
...
def migrate(source_dir: Path, actor_id: str) -> None:
    ...
    if not iocs_yaml.exists():
        ...
        iocs_yaml.write_text("# Generated by migrate-actor.py — populate indicators from iocs.md\n\n"
    ...
    print(f"   4. Run: python scripts/regenerate-ioc-index.py")
